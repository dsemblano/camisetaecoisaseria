<section class="blockhome-1 h-slide1-sm md:h-slide1-xl flex flex-col flex-no-wrap justify-between">

  <div class="blockhome-1-container container flex justify-between items-center pr-12 md:w-full pt-16 pb-2">
    @include('partials/components.btn-customize')
    @include('partials/components.dtg')
  </div>

  <div class="block1-down border-solid border-white border-t-1 tracking-widest w-full">
    <div class="container flex flex-col text-center md:text-right md:flex-row justify-between text-1xl md:text-2xl text-white">
      <span>SEM limite de cores</span>
      <span>SEM pedido mínimo</span>
      <span>100% algodão certificado</span>
      <span>Produtos à pronta entrega</span>
    </div>
  </div>

</section>

<section class="tracking-widest py-5 text-1xl text-base md:text-1xl">
  <p class="container text-center">IMAGINE | CRIE | INVENTE | IMPRIMA</p>
</section>
