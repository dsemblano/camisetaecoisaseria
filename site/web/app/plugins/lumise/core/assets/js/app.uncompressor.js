 /*
 *
 * Lumise Designer v1.7
 *
 * https://lumise.com
 * Copyright 2018 : Lumise product designer tool
	...
	var lumise = {
				...
				drawControls: function(ctx) {
					...
					if (this.hasRotatingPoint) {
						...
						if (active !== null && active.get('lockPosition') === true) {
					        ...
					        // Bottom Left for object only not group
							if (isobj)
		...
		objects : {
				...
				'mouse:move': function(opts) {
					...
					if (lumise.get.el('auto-alignment').prop('checked')) {
						...
						if (yv == 'block') {
							...
							switch (yp) {
								case 'center' :
									lumise.ops.downon.set('left', stage.limit_zone.left+(stage.limit_zone.width/2));
								break;
								case 'left' :
						...
						if (xv == 'block') {
							...
							switch (xp) {
								case 'center' :
									lumise.ops.downon.set('top', stage.limit_zone.top+(stage.limit_zone.height/2));
								break;
								case 'top' :
			...
			lumise : {
				...
				'image' : function(ops, callback) {
					...
					var new_img = fabric.util.loadImage(ops.src, function(img) {
					    ...
					    if(img == null) {
							...
							var replace_active = ops.replace;
							delete ops.replace;
		...
		design : {
			...
			events : function(){
				...
				lumise.trigger({
					...
					productColor : function(e) {
						...
						if (color && stage.limit_zone){
							...
							lumise.cart.price.color = 0;
								
							$('#lumise-product header price').html(
								lumise.func.price(lumise.cart.price.base+lumise.cart.price.color)
							);
							
							lumise.func.color_image();
					...
					saved_designs : function(e) {
						...
						var id = $(e.target).closest('li[data-id]').data('id'),
							created = $(e.target).closest('li[data-id]').data('created');
						...
						switch (act) {
							...
							case 'edit' :
								...
								if (this.getAttribute('data-view') === 'saveas') {
									lumise.tools.save('designs', id, created);
									lumise.tools.discard();
					...
					doPrint : function(e) {
						...
						let pcfg = localStorage.getItem('LUMISE_PRINT_CFG'),
						...
						switch (this.getAttribute('data-dp')) {
							...
							case 'format': 
								...
								if (format == 'pdf')
									$('#lumise-print-nav').find('li[data-row="full"]').show();
								else $('#lumise-print-nav').find('li[data-row="full"]').hide();
							...
							case 'unit':
								...
								val = $('#lumise-print-nav select[name="select-size"]').val();
						...
						if (this.tagName == 'BUTTON') {
							...
							let format = $('#lumise-print-nav input[name="print-format"]:checked').attr('data-format'),
							...
							let size = lumise.get.el('print-nav').find('input[name="size"]').val().split('x'),
					...
					doShare : function(e) {
						...
						let load_history = function(index) {
										
							let wrp = lumise.get.el('shares-wrp').find('li[data-view="history"]');
							...
							lumise.post({
								...
								if (res.per_page < res.total) {
									...
									let nav = '<ul data-view="pagenation">';
						...
						switch (func) {
							...
							case 'create-link' :
								...
								let wrp = $(e.target).closest('#lumise-shares-wrp'),
									share = {
				...
				$('#lumise-template-append').prop({checked: localStorage.getItem('LUMISE-TEMPLATE-APPEND') == 'true' ? true : false});
				$('#lumise-replace-image').prop({checked: localStorage.getItem('LUMISE-REPLACE-IMAGE') == 'true' ? true : false});
				...
				$(document).on('click', function(e){
					...
					let except = lumise.apply_filter('click_except', '');
		...
		xitems : {
			...
			scroll_more : function(e) {
				...
				if (lumise.ops[comp+'_loading'] === true)
					return;
		...
		tools : {
			...
			export : function(stage) {
				...
				data = lumise.apply_filter('export', data, stage);
			...
			toImage : function(ops) {
				...
				ops.multiplier = Math.ceil(ops.multiplier);
			...
			import : function (data, callback) {
				...
				data = lumise.apply_filter('import', data);
					...
					do_import = function(i) {
						...
						if (data.objects[i] !== undefined) {
							...
							if (typeof callback == 'function'){
								...
								if (typeof data['extra'] !== 'undefined') lumise.cart.price.extra =  data.extra;
				...
				if (lumise.data.color !== undefined) {
					...
					} else if (['standard', 'custom'].indexOf(lumise.ops.product_data.change_color)) {
		...
		func : {
				...
				switch (type) {
					...
					case 'png':
						...
						var h = ops.height,
							...
							multiplier = (h/stage.limit_zone.height) < 33 ? h/stage.limit_zone.height : 33,
							mp = o != 'landscape' ? multiplier : multiplier*(stage.limit_zone.width/stage.limit_zone.height),
							dops = {
								stage: stage,
								left: stage.limit_zone.left,
								width: stage.limit_zone.width,
								multiplier: Math.ceil(mp),
						...
						if (o != 'landscape') {
							...
							var data = lumise.tools.toImage({
								...
								width: stage.limit_zone.width,
								left: stage.limit_zone.left,
								multiplier: Math.ceil(multiplier*(stage.limit_zone.width/stage.limit_zone.height)),
					...
					case 'pdf':
						...
						var stages = Object.keys(lumise.data.stages),
							...
							do_activ = function(i) {
								lumise.active_stage(stages[i], function() {
									...
									data.push(lumise.func.export_svg(include_base));
						...
						if (window.jsPDF === undefined) {
							...
							script.onload = function(){
								lumise.func.download_design({type: 'pdf'});
								lumise.f(false);
							};
						...
						var pdf = new jsPDF('', 'mm', [210, 210*1.4151]),
							svg_string = lumise.func.export_svg().replace(/rgb?(\(\s*\d+\s*,\s*\d+\s*,\s*\d+)(?:\s*,.+?)?\)/g, '#ccc'),
							svg = $('<div>'+svg_string+'</div>').find('svg');
						console.log(svg_string.indexOf('rgb'));
						svg.find('desc,defs').remove();
						
						svgElementToPdf(svg.get(0), pdf, {
							scale: 72/96,
							removeInvalid: true
						});
						//pdf.output('datauri');
			...
			export_svg : function (include_base, is_pdf) {
				...
				lumise.stage().canvas.getObjects().
				filter(function(o){
					return o.type == 'svg';
				}).
				map(function(o, i){
			...
			update_state : function() {
				...
				$('#lumise-count-colors i').html(mo).css({background: 'linear-gradient(to right, '+bg.join(', ')+')'}).attr({title: bg.length+' colors'});
			...
			create_canvas : function(stage, img) {
				...
				if (lumise.ops.excmobile && mw > main.width()-20) {
					
					var rati = (main.width()-20)/mw;
					
					if (mh*rati > main.height()) {
						rati = main.height()/mh;
					};
					
					main.css({
						transform: 'scale('+rati+')', 
						'top': -(((mh-main.height())/2)*rati)+'px', 
						'left': -(((mw-main.width())/2)*rati)+'px'
					});
				...
				*/
				...
				var ph = stage.canvas.height*0.95,
					pw = (product.width*(stage.canvas.height/product.height))*0.95;
			...
			stage_nav : function(name, ty) {
				...
				if (name !== undefined) {
					...
					if (ty !== undefined)
						nav.attr({'data-pos': ty < 1.2 ? 'right' : 'bottom', 'data-ty': ty});
			...
			color_image : function() {
				
				var color = lumise.get.color(),
					stage = lumise.stage();
				
				if (stage.pops === undefined) {
					stage.pops = {
						width: stage.product.get('width'), 
						height: stage.product.get('height'), 
						left: stage.product.get('left'), 
						top: stage.product.get('top'), 
						originX: stage.product.get('originX'), 
						originY: stage.product.get('originY')
					}
				};
				
				try {
					
					var cs = lumise.current_stage,
						url = lumise.data.stages.colors.colors[color].images[cs].url;
					
					lumise.f('Loading..');
					stage.product.setSrc(lumise.data.upload_url+url, function() {
						stage.product.set(stage.pops);
						stage.canvas.renderAll();
						lumise.f(false);
					});
					...
					if (
						stage.product._element !== undefined && 
						stage.product.origin_src !== undefined &&
						stage.product._element.src != stage.product.origin_src
					) {
						stage.product.setSrc(stage.product.origin_src, function() {
							stage.product.set(stage.pops);
							stage.canvas.renderAll();
						});
					}
			...
			export : function(save, id, created) {
				...
				var data = {
						...
						updated: new Date().getTime()/1000,
						name : ($('#lumise-product header name t').text().trim()+' - '+lumise.stage().name).replace(/\n/g, '').trim(),
				...
				lumise.get.el('stage-nav').find('li[data-stage]').each(function(){
					...
					if (stage.canvas) {
						...
						data.stages[s] = {
							data : stage.data ? JSON.stringify(stage.data) : {},
				...
				if (
					...
					if (save == 'designs') {
						...
						data.stages = {
							'lumise': data.stages[lumise.current_stage]
						};
		...
		render : {
			...
			my_designs : function(design){
				...
				lis += '<li data-id="'+design.id+'"  data-created="'+design.created+'">\
				...
				lis += '</div>\
						<span data-view="func">\
							<i class="lumise-icon-menu"></i>\
							<span data-view="fsub">\
								<date data-func="date">'+lumise.func.date('h:m D d M, Y', design.updated*1000)+'</date>\
			...
			stage_nav : function () {
				...
				if (typeof stages == 'object') {
							...
							if (
								...
								stage_nav.push(
									'<li data-stage="'+s+'" data-tip="true"'+(s === lumise.current_stage ? ' class="active"' : '')+'>\
										<img data-stage="'+s+'" src="'+thumbn_url+'" />\
					...
					stage_nav = lumise.apply_filter('stage_nav', stage_nav);
			...
			product : function(data) {
				...
				if (color[1] !== undefined) {
					...
					color[1].map(function(c) {
						c = c.split('@');
						colors[c[0]] = {price: "", images: "", label: c[1]};
					});
					color = color[0];
				};
				...
				if (color !== undefined && color !== '' && ['standard', 'custom'].indexOf(data.change_color) > -1) {
					...
					Object.keys(colors).reverse().map(function(c) {
						
						var lb = colors[c].label;
						
						if (lb !== undefined && lb !== '')
							lb = decodeURIComponent(lb).replace(/\"/g, '');
						else if (lumise.ops.color_maps[c] !== undefined)
							lb = lumise.ops.color_maps[c];
						
						lumise.get.el('product-color')
							.prepend('<li data-color="'+c+'" title="'+lb+'" style="background:'+c+'"'+(
								c==color ? ' class="choosed"' : ''
							)+'></li>');
					});
				...
				var first_stage = lumise.render.stage_nav();
				
				lumise.render.cart_change();
				
				lumise.active_stage(first_stage, function(){
						...
						'<li>\
							<span data-view="thumbn">\
								<img style="background:'+color+'" src="'+
									item.product_data.stages[
										(Object.keys(item.product_data.stages)[0] != 'colors' ?
										Object.keys(item.product_data.stages)[0] :
										Object.keys(item.product_data.stages)[1])
									].image
								+'" />\
							</span>\
							<span data-view="info">\
		...
		post : function(ops, callback){
			...
			$.ajax({
				url: lumise.data.ajax,
				method: 'POST',
				data: lumise.apply_filter('ajax', $.extend({
					nonce: 'LUMISE-SECURITY:'+lumise.data.nonce,
					ajax: 'frontend'
				}, ops)),
			...
			list_products : function(res) {
				...
				if (res.products) {
					...
					res.products.map(function(p) {
						...
						var cates = '',
							color = p.stages.colors !== undefined ? p.stages.colors.active : p.color.split(':')[0];
						...
						prods.push(
							'<li data-id="'+p.id+'"'+cates+(
								...
								<span data-view="thumbn" data-start="'+btn_text+'">\
									<img style="background:'+color+'" src="'+
									p.stages[
										Object.keys(p.stages)[0] != 'colors' ? 
										Object.keys(p.stages)[0] : 
										Object.keys(p.stages)[1]
									].image+'" />\
								</span>\
		...
		active_stage : function(name, callback) {
			...
			fabric.util.loadImage(stage.image, function(img) {
			    ...
			    else 
			    {
					...
					/*
					*	if product color has custom image
					*/
					...
					if (stage.data) {
						...
						lumise.templates.load(stage.template, callback); 
					} else {
						lumise.stack.save();
						if (lumise.ops.first_completed !== true) {
							lumise.actions.do('first-completed');
		...
		cart : {
			...
			sum : function (){
				...
				var extra_filter = lumise.apply_filter('product_extra_price', {}),
			...
			extra_price : function ( ext_id, data ) {
				...
				if( typeof lumise.cart.price.extra[ext_id] === 'undefined' ) lumise.cart.price.extra[ext_id] = [];
				//check exists id of resource
				var f = lumise.cart.price.extra[ext_id].filter(
					obj => obj.id == data.id && obj.table == data.table
				);
				
				if( typeof f[0] === 'undefined' ) lumise.cart.price.extra[ext_id].push(data);
			},
			...
			add_cart : function(e) {
				...
				*/
				...
				if (lumise.cart.qty == 0){
					invalid_fields.push(attrs.find('.lumise-cart-field-size-tmpl,.lumise-cart-field-pack-tmpl').get(0));
					attrs.find('.lumise-cart-field-size-tmpl .lumise-required-msg, .lumise-cart-field-pack-tmpl .lumise-required-msg').html(lumise.i(103));
				};
				
				if (lumise.ops.product_data.min_qty !== '' && parseInt(lumise.ops.product_data.min_qty) > lumise.cart.qty) {
					invalid_fields.push(attrs.find('.lumise-cart-field-size-tmpl').get(0));
					attrs.find('.lumise-cart-field-size-tmpl .lumise-required-msg').html(lumise.i(149)+' : '+lumise.ops.product_data.min_qty);
				}
			...
			calc : function (states_data) {
				...
				var price = 0,
					...
					values = $('.lumise-cart-attributes').find('.lumise-cart-param:not(.disabled)').serializeArray(),
					keys = [],
					color = lumise.get.color();
				...
				Object.keys(lumise.ops.product_data.stages).map(function(s){
					if( 
						s === 'colors' &&
				 		typeof lumise.ops.product_data.stages[s].colors !== 'undefined'
					){
						var colors = lumise.ops.product_data.stages[s].colors;
						
						Object.keys(colors).map(function (c){
							if( color == c && colors[c].price !== '' ){
								price += parseFloat(colors[c].price);
							}
						});
					}
				});
			...
			*/
			...
			fields : {
					...
					radio : function(data) {
						...
						for(var i in data.options){
							...
							var option = data.options[i],
								...
								price = parseFloat(option.price) > 0? parseFloat(option.price) : '';
					...
					pack : function (data){
						...
						data.options.map(function(op){
							radios += '<div class="lumise-radio">\
	                			<input type="radio" '+(
		                			data.value.indexOf(op.quantity) > -1 ? 'checked' : ''
	                			)+' class="lumise-cart-param" name="'+data.name+'" value="'+op.quantity+'" id="'+data.name+'-'+op.quantity+'">\
				                <label class="lumise-cart-option-label" for="'+data.name+'-'+op.quantity+'">\
				                	<strong>'+op.quantity+'</strong>'+(op.title.trim() !== '' ? ' ('+op.title+')' : '')+
				                	' <em class="check"></em>\
				                </label>\
				...
				condition : function() {
					...
					var is_condi = function(el, vals) {
						...
						if (el.tagName == 'INPUT') {
							if (
								...
								el.checked === true && 
								vals.indexOf(el.value) > -1
							)
								re = true;
							else if (vals.indexOf(el.value) > -1)
								re = true;
						...
						} else if (el.tagName == 'TEXTAREA' && vals.indexOf(el.value) > -1)
							re = true;
					...
					$('#lumise-cart-wrp .lumise-cart-attributes>div.lumise-cart-field[data-condition]').each(function() {
						
						var el = $(this),
							vals = el.attr('data-condition-value').split("%0A"),
							type = el.attr('data-condition'),
							parent = el.attr('data-condition-parent');
						
						$('#lumise-cart-wrp .lumise-cart-attributes>div.lumise-cart-field[data-id="'+parent+'"]')
						.find('.lumise-cart-param').each(function() {
							
							if ($(this).closest('div.lumise-cart-field').hasClass('disabled')) {
								el.hide().addClass('disabled');
								return;
							}
							
							var con = is_condi(this, vals);
							
							if (type == 'show') {
								if (con)
									el.show().removeClass('disabled');
								else el.hide().addClass('disabled');
							} else if (type == 'hide') {
								if (con)
									el.hide().addClass('disabled');
								else el.show().removeClass('disabled');
							}
				...
				render : function(data) {
					...
					if (typeof this.renders[data.type] == 'function') {
						var el = this.renders[data.type](data);
						el.attr({
							'data-id': data.id
						});
						if (data.condition !== undefined && data.condition !== false) {
							el.attr({
								...
								'data-condition-parent': data.condition.parent,
								'data-condition-value': data.condition.value
							});
						}
					} else {
						el = $('<p>Error: field not found <b>'+data.type+'</b></p>');
					};
	        ...
	        printing : {
				...
				render : function (val){
					...
					var data = {
						...
						field = lumise.cart.fields.renders._template(data),
						wop = field.el.find('.lumise_radios'),
						fel = field.el.find('.lumise-radio'),
						first_inp = null;
	        ...
	        edit_item : function (id, e) {
				...
				if (data && data[id] !== undefined) {
					...
					lumise.actions.do('cart_edit', lumise.apply_filter('cart_edit', cart));
					delete data;
				};
		        	...
		        	var data = JSON.parse(localStorage.getItem('LUMISE-CART-DATA')),
		        		...
		        		get_design = function(res){
			        		...
			        		if (count === Object.keys(data).length) {
				        		lumise.actions.do('checkout', lumise.apply_filter('checkout', data));
			        		}else lumise.f(false);
		...
		load : function() {
			...
			*/
				...
				if (lumise.func.url_var('cart', '') != '') {
							...
							statusCode: {
								...
								200: function(res) {
									...
									lumise.tools.imports(res, function(){
										...
										//add hook for action after popup print open
										lumise.do_action('after_print_menu_trigger');
									});
								}
							}
						});
					...
					lumise.indexed.get(lumise.data.product, 'drafts', function(res){
						...
						if (res !== null && res !== undefined && res.stages !== undefined){
							...
							if (count > 2)
								scrs += '<li data-view="more">+'+(count-2)+'</li>';
			...
			this.actions.add('draft-autosaved', function(){
				...
				txt += '<a title="'+lumise.i(114)+'" href="'+
							window.location.href.replace('&draft=yes','').replace('?draft=yes&','?').replace('?draft=yes','')+
						'">\
