...
(function($){
    ...
    function _add_real_change_price(e,v) {
        pc.products[$('.pc-product.pc-change-price').attr('data-id')][$('.pc-product.pc-change-price .pc-price.pc-change-price').attr('data-type')] = v;
        get_products(false);
    }
    ...
    function _make_change_price_input(e) {
        $(e.target).append('<input id="pc-change-value" type="text" />');
        $('#pc-change-value').focus();
    }
    ...
    function display_pagination(p) {
        ...
        if ( Math.ceil(p.total/p.posts_per_page) > 0 ) {
            for( var i=0; i<Math.ceil(p.total/p.posts_per_page); i++ ) {
                $('#pc-pagination').append( '<span class="pc-pagination'+(p.paged==i+1?' pc-active':'')+'" data-page="'+(i+1)+'">'+(i+1)+'</span>' );
            }
        }
    }
    ...
    function _call_ajax_pagination(e) {
        pc.execute = {};
        pc.query.paged = e.target.dataset.page;
